...
```html
<section class="filter-bar" aria-label="Filtros da lista">
  <div class="filter-bar__surface">
    <label class="sr-only" for="filter-query">Buscar casos</label>
    <input id="filter-query" placeholder="Buscar caso, fonte ou termo" />

    <div class="filter-bar__period" aria-label="Período">
      <button type="button" aria-pressed="false">24h</button>
    ...
    <button type="button" class="filter-bar__facet">Risco <span>1</span></button>
    <button type="button" class="filter-bar__facet">Fonte</button>
  </div>

  <div class="filter-bar__active" aria-label="Filtros ativos">
    <button type="button"><span>Risco</span> high <span aria-hidden="true">×</span></button>
    <button type="button"><span>Origem</span> Cris <span aria-hidden="true">×</span></button>
  </div>
</section>
```
...
.filter-bar__period button[aria-pressed="true"] {
  background: var(--card);
  color: var(--fg);
